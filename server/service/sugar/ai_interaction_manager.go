...
- **维度排序重要示例**：
	 * 货币资金分析：['银行名称', '账户类型', '币种']
	 * 固定资产分析：['使用部门', '资产类型', '折旧年限'] （部门→资产→属性的逻辑顺序）
	 * 销售分析：['地区', '产品类别', '销售渠道']
...
---
**匿名化数据处理指南:**
- **数据已脱敏**: 你接收到的数据中，维度名称（如 DIM01、DIM02）和维度值（如 DIM01_V01）都经过了匿名化处理。
- **维度语义理解**: 数据开头包含"维度代号说明"，格式为"DIM01-折旧年限"，请仔细阅读这些说明来理解每个维度代号的真实含义。
- **语序组织核心原则**:
	 * **按业务逻辑排序，不要按DIM编号排序**：根据维度的业务含义组织语言，例如"部门的资产类型"而不是"资产类型的部门"
	 * **常见业务逻辑顺序**：组织机构 → 业务分类 → 具体属性（如：研发部 → 电子设备 → 5年折旧）
	 * **使用自然连接词**：优先使用"的"连接，避免"中"、"内"等容易造成歧义的词汇
- **表达优化建议**:
	 * 优先表达为"A的B"而不是"A中的B"或"A内B"
	 * 对于多个维度，按从大到小的业务层级组织：如"研发部的电子设备"
- **关注相对关系**: 分析的重点应放在数据的模式、趋势和相对贡献度上，而不是具体的绝对值。
- **贡献度分析**: 数据中包含了"贡献度百分比"和"是否为正向驱动"，请利用这些信息来识别关键影响因素。
- **还原业务含义**: 在输出最终结论时，系统会自动将匿名代号解码回真实的业务术语，所以请在分析时大胆使用这些代号，并根据维度说明合理组织表达顺序。
`
			...
			Parameters: map[string]interface{}{
				...
				"properties": map[string]interface{}{
					...
					"groupByDimensions": map[string]interface{}{
						...
						"description": "进行分组和归因分析的维度列名列表，如 ['区域', '产品类别']。**重要：请按照业务逻辑的语义顺序排列维度，这样有利于后续匿名化还原时保持语句的通顺性。常见排序原则：组织机构→业务分类→具体属性。示例：固定资产分析应按 ['使用部门', '资产类型', '折旧年限'] 顺序（部门→资产→属性），货币资金分析应按 ['银行名称', '账户类型', '币种'] 顺序。**",
					},
