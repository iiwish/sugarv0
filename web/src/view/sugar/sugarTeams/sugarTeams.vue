  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column type="selection" width="55" />
            ...
            <el-table-column align="left" label="teamName字段" prop="teamName" width="120" />

            <el-table-column align="left" label="团队创建者/个人空间的所有者" prop="ownerId" width="120" />

            <el-table-column align="left" label="是否为个人空间团队 (true代表个人空间)" prop="isPersonal" width="120">
    <template #default="scope">{{ formatBoolean(scope.row.isPersonal) }}</template>
</el-table-column>
            <el-table-column align="left" label="createdBy字段" prop="createdBy" width="120" />

            <el-table-column align="left" label="createdAt字段" prop="createdAt" width="180">
   <template #default="scope">{{ formatDate(scope.row.createdAt) }}</template>
</el-table-column>
            <el-table-column align="left" label="updatedBy字段" prop="updatedBy" width="120" />
   ...
   <template #default="scope">{{ formatDate(scope.row.updatedAt) }}</template>
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateSugarTeamsFunc(scope.row)">编辑</el-button>
    ...
    {{ detailForm.id }}
                    ...
                    <el-descriptions-item label="teamName字段">
    {{ detailForm.teamName }}
</el-descriptions-item>
                    <el-descriptions-item label="团队创建者/个人空间的所有者">
    {{ detailForm.ownerId }}
</el-descriptions-item>
                    <el-descriptions-item label="是否为个人空间团队 (true代表个人空间)">
    {{ detailForm.isPersonal }}
</el-descriptions-item>
                    <el-descriptions-item label="createdBy字段">
    {{ detailForm.createdBy }}
</el-descriptions-item>
                    <el-descriptions-item label="createdAt字段">
    {{ detailForm.createdAt }}
</el-descriptions-item>
                    <el-descriptions-item label="updatedBy字段">
...
const onSubmit = () => {
  elSearchFormRef.value?.validate(async(valid) => {
    if (!valid) return
    page.value = 1
    if (searchInfo.value.isPersonal === ""){
        searchInfo.value.isPersonal=null
    }
    getTableData()
  })
}
...
const getTableData = async() => {
  const table = await getSugarTeamsList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteSugarTeamsByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateSugarTeamsFunc = async(row) => {
    const res = await findSugarTeams({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteSugarTeamsFunc = async (row) => {
    const res = await deleteSugarTeams({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
