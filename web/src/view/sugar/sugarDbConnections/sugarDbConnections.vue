  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column type="selection" width="55" />
            ...
            <el-table-column align="left" label="teamId字段" prop="teamId" width="120" />

            <el-table-column align="left" label="dbType字段" prop="dbType" width="120" />

            <el-table-column align="left" label="是否为Sugar内部数据库 (true代表本库)" prop="isInternal" width="120">
    <template #default="scope">{{ formatBoolean(scope.row.isInternal) }}</template>
</el-table-column>
            <el-table-column align="left" label="内部数据库可为null" prop="host" width="120" />

            <el-table-column align="left" label="内部数据库可为null" prop="port" width="120" />

            <el-table-column align="left" label="内部数据库可为null" prop="username" width="120" />

            <el-table-column align="left" label="内部数据库可为null" prop="encryptedPassword" width="120" />

            <el-table-column align="left" label="databaseName字段" prop="databaseName" width="120" />

            <el-table-column align="left" label="sslConfig字段" prop="sslConfig" width="120" />

        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            <template #default="scope">
            <el-button  type="primary" link class="table-button" @click="getDetails(scope.row)"><el-icon style="margin-right: 5px"><InfoFilled /></el-icon>查看</el-button>
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateSugarDbConnectionsFunc(scope.row)">编辑</el-button>
    ...
    {{ detailForm.name }}
                    ...
                    <el-descriptions-item label="teamId字段">
    {{ detailForm.teamId }}
</el-descriptions-item>
                    <el-descriptions-item label="dbType字段">
    {{ detailForm.dbType }}
</el-descriptions-item>
                    <el-descriptions-item label="是否为Sugar内部数据库 (true代表本库)">
    {{ detailForm.isInternal }}
</el-descriptions-item>
                    <el-descriptions-item label="内部数据库可为null">
    {{ detailForm.host }}
</el-descriptions-item>
                    <el-descriptions-item label="内部数据库可为null">
    {{ detailForm.port }}
    ...
    {{ detailForm.username }}
                    ...
                    <el-descriptions-item label="内部数据库可为null">
    {{ detailForm.encryptedPassword }}
</el-descriptions-item>
                    <el-descriptions-item label="databaseName字段">
    {{ detailForm.databaseName }}
</el-descriptions-item>
                    <el-descriptions-item label="sslConfig字段">
    {{ detailForm.sslConfig }}
</el-descriptions-item>
            </el-descriptions>
        </el-drawer>
...
import {
  ...
  deleteSugarDbConnections,
  deleteSugarDbConnectionsByIds,
  updateSugarDbConnections,
  findSugarDbConnections,
  getSugarDbConnectionsList
...
const onSubmit = () => {
  elSearchFormRef.value?.validate(async(valid) => {
    if (!valid) return
    page.value = 1
    if (searchInfo.value.isInternal === ""){
        searchInfo.value.isInternal=null
    }
    getTableData()
  })
}
...
const getTableData = async() => {
  const table = await getSugarDbConnectionsList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteSugarDbConnectionsFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteSugarDbConnectionsByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateSugarDbConnectionsFunc = async(row) => {
    const res = await findSugarDbConnections({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteSugarDbConnectionsFunc = async (row) => {
    const res = await deleteSugarDbConnections({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
