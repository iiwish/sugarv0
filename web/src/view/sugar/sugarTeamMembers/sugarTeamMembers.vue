...
<template>
  ...
  <el-input v-model="searchInfo.teamId" placeholder="搜索条件" />
</el-form-item>
  ...
  <el-input v-model="searchInfo.userId" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column type="selection" width="55" />
            ...
            <el-table-column align="left" label="role字段" prop="role" width="120" />

        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            <template #default="scope">
            <el-button  type="primary" link class="table-button" @click="getDetails(scope.row)"><el-icon style="margin-right: 5px"><InfoFilled /></el-icon>查看</el-button>
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateSugarTeamMembersFunc(scope.row)">编辑</el-button>
    ...
    {{ detailForm.userId }}
                    ...
                    <el-descriptions-item label="role字段">
    {{ detailForm.role }}
</el-descriptions-item>
            </el-descriptions>
        </el-drawer>
...
import {
  ...
  deleteSugarTeamMembers,
  deleteSugarTeamMembersByIds,
  updateSugarTeamMembers,
  findSugarTeamMembers,
  getSugarTeamMembersList
...
const getTableData = async() => {
  const table = await getSugarTeamMembersList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteSugarTeamMembersFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteSugarTeamMembersByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateSugarTeamMembersFunc = async(row) => {
    const res = await findSugarTeamMembers({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteSugarTeamMembersFunc = async (row) => {
    const res = await deleteSugarTeamMembers({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
