  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column type="selection" width="55" />
            ...
            <el-table-column align="left" label="系统预置, 团队自定义" prop="agentType" width="120" />

            <el-table-column align="left" label="teamId字段" prop="teamId" width="120" />

            <el-table-column align="left" label="定义 Agent 的调用方式, 如 API URL, headers 等" prop="endpointConfig" width="120" />

        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            <template #default="scope">
            <el-button  type="primary" link class="table-button" @click="getDetails(scope.row)"><el-icon style="margin-right: 5px"><InfoFilled /></el-icon>查看</el-button>
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateSugarAgentsFunc(scope.row)">编辑</el-button>
    ...
    {{ detailForm.description }}
                    ...
                    <el-descriptions-item label="系统预置, 团队自定义">
    {{ detailForm.agentType }}
</el-descriptions-item>
                    <el-descriptions-item label="teamId字段">
    {{ detailForm.teamId }}
</el-descriptions-item>
                    <el-descriptions-item label="定义 Agent 的调用方式, 如 API URL, headers 等">
    {{ detailForm.endpointConfig }}
</el-descriptions-item>
            </el-descriptions>
        </el-drawer>
...
const getTableData = async() => {
  const table = await getSugarAgentsList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteSugarAgentsByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateSugarAgentsFunc = async(row) => {
    const res = await findSugarAgents({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteSugarAgentsFunc = async (row) => {
    const res = await deleteSugarAgents({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
